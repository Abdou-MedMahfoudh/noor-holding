...
function newsCardTemplate(item) {
  const imagePart = item.picture
    ? `<div class="h-48"><img src="${item.picture}" alt="${i18n.tn(
        `articles.${item.id}.title`
    ...
    <div class="p-6">
      ...
      <div class="flex items-center space-x-2">
        <a href="./news.html?id=${
          ...
          ${i18n.t("news.readMore") || "Voir plus"}
        </a>
...
async function renderLatestNews(containerId = "news-container", count = 3) {
  ...
  try {
    ...
    renderInto(
      container,
      `<div class="text-center text-gray-500">Unable to load news at the moment.</div>`
    );
  }
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  languageButtons.forEach((btn) => {
    btn.addEventListener("click", async () => {
      await renderLatestNews(); // re-render news in new language
    });
  });
